...
The default threshold of 5 methods works well for most projects, but you can adjust it in your `great-docs.yml`:
...
### Documenting Class Methods Separately

For classes with many methods, you can document methods on separate pages by using `members: separate`:
...
When `members: separate` is set, Great Docs automatically creates a "[ClassName] Methods" section with individual pages for each method.
