...
1. Create a `user_guide/` directory in your project root (not in `docs/`)
2. Add `.qmd` files for each page
3. Run `great-docs build`

That's it! Great Docs automatically:

- Copies files to `docs/user-guide/`
...
your-project/
├── docs/                    # Generated docs (don't edit directly)
├── user_guide/              # Your source files
│   ├── 00-introduction.qmd
│   ├── 01-installation.qmd
│   ├── 02-quickstart.qmd
...
For Python code that you want to actually execute and show the output, use `{python}` instead of just `python`:
...
::: {.callout-note}
Executable code blocks require Jupyter. Install it with `pip install jupyter` or install Great Docs with the docs extras: `pip install great-docs[docs]`.
:::

You can control execution behavior with hash-pipe options at the top of the code block:
...
print("This code is shown, executed, and its output displayed")
```
~~~

Some useful hash-pipe options include:
...
```markdown
See the [Installation](01-installation.qmd) guide for details.
```
...
user_guide/
├── 01-guide.qmd
├── images/          # Copied to docs/user-guide/images/
│   ├── logo.png
│   └── diagram.svg
└── data/            # Copied to docs/user-guide/data/
    └── example.json
```
...
### Keep Source in Project Root

The `user_guide/` directory should be in your project root, not inside `docs/`. Great Docs copies files during build, so your source stays separate from generated output.
...
- **[Deployment](06-deployment.qmd)** – Publish your documentation to GitHub Pages
