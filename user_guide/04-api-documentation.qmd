...
When set, the `title` replaces the default "Reference" heading on the API index page and in the navigation bar. The `desc` text appears as a paragraph immediately below the heading, providing context before the section listings. Sections are auto-generated from your package's public API.

If neither `title` nor `desc` is set, the page heading defaults to "Reference" with no introductory text.

:::{.callout-note}
The `title`/`desc` form and the explicit section list form are separate ways to use the `reference` key. Use `reference:` as a dict (with `title` and `desc`) for a custom heading with auto-discovered sections, or as a list of section dicts for full control over organization.
:::
