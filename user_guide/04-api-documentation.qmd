...
- **Shortened**: write `` [](`~mypackage.MyClass`) `` to display just `MyClass`. The `~` prefix strips the package path and shows only the short name.
- **Fully qualified**: write `` [](`mypackage.MyClass`) `` to display the full path `mypackage.MyClass`.
- **Custom text**: write `` [see this class](`mypackage.MyClass`) `` to display `see this class` as the link text.
- **Custom text + tilde**: write `` [see this class](`~mypackage.MyClass`) `` — this also displays `see this class`. When you supply custom link text in the brackets, it's always used as-is regardless of the `~` prefix.
...
- **Full path**: `` `mypackage.MyClass` `` displays as `mypackage.MyClass`.
- **Full path with parens**: `` `mypackage.my_func()` `` displays as `mypackage.my_func()`.
- **Shortened**: `` `~~mypackage.MyClass` `` displays as `MyClass`. The `~~` prefix strips everything before the last component.
- **Shortened with parens**: `` `~~mypackage.my_func()` `` displays as `my_func()`.
- **Dot-prefixed short**: `` `~~.mypackage.MyClass` `` displays as `.MyClass`, retaining a leading dot to hint at the omitted path.
- **Unmatched (no link)**: if the name doesn't match any documented symbol (e.g., `` `~~unknown_func()` ``), it renders as plain code (`unknown_func()`) with no link.
