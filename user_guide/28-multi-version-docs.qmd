...
- The current version (with a checkmark)
- Pre-release versions (with a dot indicator)
- End-of-life versions (with a warning indicator)

When switching versions, the widget navigates to the same page in the target version. If that page doesn't exist (because it was added or removed between versions), it falls back to the section index, then the homepage.
