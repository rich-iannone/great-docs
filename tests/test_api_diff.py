...
class TestSymbolChangeToDict:
    ...
    def test_full(self):
        sc = SymbolChange(
            ...
            parameter_changes=[
                ParameterChange(parameter="a", change_type="removed", is_breaking=True)
            ],
            is_breaking=True,
...
class TestDescribeParamChange:
    ...
    def test_descriptions(self, change_type, expected_substr):
        pc = ParameterChange(
            parameter="x",
...
class TestDiffParameters:
    ...
    def test_added_no_default_is_breaking(self):
        old = [ParameterInfo(name="x")]
    ...
    def test_kind_changed_is_breaking(self):
...
class TestDiffSymbol:
    ...
    def test_base_removed_is_breaking(self):
    ...
    def test_decorator_changed(self):
    ...
    def test_parameter_changes_propagated(self):
        old = SymbolInfo(
            name="foo",
            kind="function",
...
class TestDiffSnapshots:
    def _snap(self, version: str, symbols: dict[str, SymbolInfo]) -> ApiSnapshot:
        return ApiSnapshot(version=version, package_name="pkg", symbols=symbols)

    def test_empty_to_empty(self):
        diff = diff_snapshots(self._snap("v1", {}), self._snap("v2", {}))
    ...
    def test_added(self):
        ...
        diff = diff_snapshots(self._snap("v1", {}), self._snap("v2", new_symbols))
    ...
    def test_removed(self):
        ...
        diff = diff_snapshots(self._snap("v1", old_symbols), self._snap("v2", {}))
    ...
    def test_changed(self):
        ...
        diff = diff_snapshots(self._snap("v1", old_symbols), self._snap("v2", new_symbols))
...
class TestDependencyGraph:
    ...
    def test_mermaid_with_nodes(self):
        g = DependencyGraph(
    ...
    def test_mermaid_call_edge(self):
        g = DependencyGraph(
...
class TestBuildDependencyGraph:
    ...
    def test_external_base_excluded(self):
        snap = ApiSnapshot(
            version="v1",
            package_name="pkg",
            symbols={
    ...
    def test_all_symbols_become_nodes(self):
        snap = ApiSnapshot(
            version="v1",
            package_name="pkg",
            symbols={
...
class TestListVersionTags:
    @patch("great_docs._api_diff.subprocess.run")
    def test_parses_tags(self, mock_run):
    ...
    @patch("great_docs._api_diff.subprocess.run")
    def test_handles_no_v_prefix(self, mock_run):
...
# ── Snapshot from griffe ─────────────────────────────────────────────────


class TestSnapshotFromGriffe:
    def test_snapshot_of_real_package(self):
    ...
    def test_snapshot_includes_parameters(self):
        snap = snapshot_from_griffe("great_docs", version="test")
    ...
    def test_snapshot_nonexistent_package(self):
...
class TestFormatSignature:
    ...
    def test_attribute_no_annotation(self):
...
class TestSymbolHistory:
    ...
    def test_changed_entries_first_present(self):
        hist = SymbolHistory(
            symbol_name="foo",
            package_name="pkg",
            entries=[
    ...
    def test_changed_entries_with_change(self):
        ...
        hist = SymbolHistory(
            ...
            entries=[
                self._entry("v1"),
                self._entry("v2", change=diff_change),
                self._entry("v3"),
            ],
        )
        changed = hist.changed_entries
    ...
    def test_changed_entries_removal(self):
        ...
        hist = SymbolHistory(
            ...
            entries=[
                self._entry("v1"),
                self._entry("v2", present=False, sig=None, change=removal),
            ],
        )
        changed = hist.changed_entries
    ...
    def test_changed_entries_reappearance(self):
        hist = SymbolHistory(
            symbol_name="foo",
            package_name="pkg",
            entries=[
                self._entry("v1"),
                self._entry("v2", present=False, sig=None),
                self._entry("v3"),  # reappeared
            ],
        )
        changed = hist.changed_entries
    ...
    def test_to_dict_all(self):
        hist = SymbolHistory(
            symbol_name="foo",
            package_name="pkg",
            entries=[self._entry("v1"), self._entry("v2")],
        )
        d = hist.to_dict()
    ...
    def test_to_dict_changes_only(self):
        hist = SymbolHistory(
            symbol_name="foo",
            package_name="pkg",
            entries=[self._entry("v1"), self._entry("v2"), self._entry("v3")],
        )
...
class TestSymbolHistoryFunction:
    @patch("great_docs._api_diff.list_version_tags")
    @patch("great_docs._api_diff.snapshot_at_tag")
    def test_tracks_across_versions(self, mock_snap, mock_tags):
    ...
    @patch("great_docs._api_diff.list_version_tags")
    @patch("great_docs._api_diff.snapshot_at_tag")
    def test_symbol_not_present(self, mock_snap, mock_tags):
    ...
    @patch("great_docs._api_diff.list_version_tags")
    @patch("great_docs._api_diff.snapshot_at_tag")
    def test_symbol_added_later(self, mock_snap, mock_tags):
        mock_tags.return_value = ["v1", "v2"]
    ...
    @patch("great_docs._api_diff.list_version_tags")
    @patch("great_docs._api_diff.snapshot_at_tag")
    def test_symbol_removed(self, mock_snap, mock_tags):
        mock_tags.return_value = ["v1", "v2"]
    ...
    def test_returns_none_without_package(self, tmp_path):
...
class TestApiDiffCLI:
    ...
    def test_help_includes_table_html(self):
        from click.testing import CliRunner
...
class TestEvolutionTable:
    ...
    def test_parameter_name_in_cell(self):
    ...
    def test_type_and_default_in_cell(self):
    ...
    def test_absent_slot_shows_dash(self):
    ...
    def test_changes_only_filters(self):
        hist = _make_history()
    ...
    def test_type_only_no_default(self):
...
class TestEvolutionTableText:
    ...
    def test_version_labels_in_header(self):
        hist = _make_history()
        text = evolution_table_text(hist, changes_only=False)
...
class TestEvolutionTableHtml:
    ...
    def test_name_and_type_spans(self):
    ...
    def test_disclosure_wrapping(self):
        hist = _make_history()
    ...
    def test_custom_summary(self):
        ...
        html = evolution_table_html(
            hist, changes_only=False, disclosure=True, summary_text="History"
        )
...
class TestEvolutionTableToDict:
    ...
    def test_cells_parallel_to_versions(self):
        hist = _make_history()
        d = evolution_table_to_dict(hist, changes_only=False)
    ...
    def test_null_cell_for_absent_slot(self):
        hist = _make_history()
        d = evolution_table_to_dict(hist, changes_only=False)
    ...
    def test_cell_with_default(self):
        hist = _make_history()
        d = evolution_table_to_dict(hist, changes_only=False)
    ...
    def test_no_return_type_omitted(self):
    ...
    def test_matches_demo_json_structure(self):
        """Verify our output matches the documented JSON spec."""
        demo_path = (
            Path(__file__).parent.parent / "great_docs" / "assets" / "api-evolution-demo.json"
        )
...
class TestAugmentParamsWithSeparators:
    def test_no_keyword_only(self):
    ...
    def test_no_star_when_var_positional(self):
...
class TestEvolutionTableStarSeparator:
    def _make_kw_history(self):
    ...
    def test_star_row_in_table(self):
        hist = self._make_kw_history()
        rows = evolution_table(hist, changes_only=False)
    ...
    def test_star_separator_in_html(self):
        hist = self._make_kw_history()
        html = evolution_table_html(hist, changes_only=False)
...
class TestReturnsLabel:
    def test_returns_label_in_table(self):
        hist = _make_history()
        rows = evolution_table(hist, changes_only=False)
    ...
    def test_returns_label_in_html(self):
        hist = _make_history()
        html = evolution_table_html(hist, changes_only=False)
...
class TestDateTooltips:
    ...
    def test_no_tooltip_when_no_date(self):
        sym = SymbolInfo(name="fn", kind="function", parameters=[])
        hist = SymbolHistory(
            symbol_name="fn",
            package_name="pkg",
            entries=[
                SymbolHistoryEntry(
                    version="v1", present=True, signature="def fn()", symbol_info=sym
                ),
            ],
        )
        html = evolution_table_html(hist, changes_only=False)
...
# ── Filter tag range tests ─────────────────────────────────────────────


class TestFilterTagRange:
    ...
    def test_missing_old_returns_all(self):
        tags = ["v1", "v2"]
        assert _filter_tag_range(tags, "v99", None) == ["v1", "v2"]

    def test_missing_new_returns_all(self):
        tags = ["v1", "v2"]
        assert _filter_tag_range(tags, None, "v99") == ["v1", "v2"]


# ── Render evolution table (high-level) ────────────────────────────────


class TestRenderEvolutionTable:
    @patch("great_docs._api_diff.list_version_tags")
    def test_no_tags(self, mock_tags):
    ...
    @patch("great_docs._api_diff.list_version_tags")
    def test_no_history(self, mock_tags, mock_hist):
        mock_tags.return_value = ["v1", "v2"]
    ...
    @patch("great_docs._api_diff.symbol_history")
    @patch("great_docs._api_diff.list_version_tags")
    def test_returns_css_and_table(self, mock_tags, mock_hist):
        mock_tags.return_value = ["v1", "v2"]
        sym = SymbolInfo(
            name="build",
            kind="function",
    ...
    @patch("great_docs._api_diff.symbol_history")
    @patch("great_docs._api_diff.list_version_tags")
    def test_include_css_false(self, mock_tags, mock_hist):
    ...
    @patch("great_docs._api_diff.symbol_history")
    @patch("great_docs._api_diff.list_version_tags")
    def test_disclosure_false(self, mock_tags, mock_hist):
        mock_tags.return_value = ["v1"]
        sym = SymbolInfo(
            name="fn",
            kind="function",
    ...
    @patch("great_docs._api_diff.symbol_history")
    @patch("great_docs._api_diff.list_version_tags")
    def test_tag_range_filtering(self, mock_tags, mock_hist):
    ...
    def test_css_constant_has_all_classes(self):
...
class TestProcessEvolutionMarkers:
    @patch("great_docs._api_diff.render_evolution_table")
    def test_replaces_marker(self, mock_render):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_passes_attributes(self, mock_render):
    ...
    def test_missing_symbol_gives_error(self):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_multiple_markers(self, mock_render):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_no_markers_returns_unchanged(self, mock_render):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_exception_gives_error_comment(self, mock_render):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_summary_attr(self, mock_render):
        mock_render.return_value = "<table/>"
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_package_attr_overrides_argument(self, mock_render):
        mock_render.return_value = "<table/>"
...
class TestProcessEvolutionMarkersInFile:
    @patch("great_docs._api_diff.render_evolution_table")
    def test_reads_and_processes(self, mock_render, tmp_path):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_in_place(self, mock_render, tmp_path):
    ...
    @patch("great_docs._api_diff.render_evolution_table")
    def test_no_markers_skips_write(self, mock_render, tmp_path):
        qmd = tmp_path / "test.qmd"
...
# ── Evolution shortcode extension structure ──────────────────────────────


class TestEvolutionShortcodeExtension:
    """Verify the Quarto extension files exist and are well-formed."""
    ...
    def test_lua_filter_exists(self):
        assert (self._ext_dir() / "evolution.lua").exists()

    def test_python_bridge_exists(self):
        assert (self._ext_dir() / "_evolution_shortcode.py").exists()
    ...
    def test_lua_defines_evolution_function(self):
        lua_src = (self._ext_dir() / "evolution.lua").read_text()
        # Must return a table with an "evolution" key
        assert '["evolution"]' in lua_src
        assert "function(args, kwargs)" in lua_src

    def test_python_bridge_parses_args(self):
        """The bridge script can parse its arguments without errors."""
        import subprocess
        import sys

        bridge = self._ext_dir() / "_evolution_shortcode.py"
...
# ── render_evolution_table_from_dict tests ──────────────────────────────


class TestRenderEvolutionTableFromDict:
    """Tests for rendering evolution tables from JSON-compatible dicts."""
    ...
    def test_basic_rendering(self):
        html = render_evolution_table_from_dict(self._base_data())
        assert "gd-evolution-table" in html
        assert "v1.0" in html
        assert "v2.0" in html
        assert "path" in html

    def test_empty_versions_returns_comment(self):
    ...
    def test_css_excluded(self):
        html = render_evolution_table_from_dict(self._base_data(), include_css=False)
        assert "<style>" not in html
        assert "gd-evolution-table" in html

    def test_disclosure_wrapper(self):
        html = render_evolution_table_from_dict(self._base_data(), disclosure=True)
        assert "<details" in html
        assert "<summary>" in html
    ...
    def test_custom_summary(self):
        html = render_evolution_table_from_dict(
            self._base_data(), disclosure=True, summary_text="My Title"
        )
        assert "My Title" in html

    def test_null_cells_render_absent(self):
        data = {
            "symbol": "f",
            "versions": ["v1", "v2"],
            "slots": [
    ...
    def test_dates_as_tooltips(self):
        data = self._base_data()
    ...
    def test_roundtrip_with_demo_json(self):
